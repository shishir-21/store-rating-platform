...
router.get('/dashboard', async (req, res, next) => {
  try {
    ...
    const storeResult = await pool.query('SELECT id,name,ROUND(AVG(r.score)::numeric,2) average_rating FROM stores s LEFT JOIN ratings r ON r.store_id=s.id WHERE s.owner_id=$1 GROUP BY s.id', [req.user.id]);
